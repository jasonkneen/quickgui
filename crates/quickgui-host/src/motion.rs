//! GPUIX motion semantics projected through QuickGUI declarative animations.
//! Adapted from remorses/gpuix packages/native/src/motion.rs (Apache-2.0).
...
use quickgui::{Animation, AnimationExt, Element};
...
fn parse_description(source: &serde_json::Value) -> Result<MotionDescription, String> {
    ...
    validate_seconds(
        description.transition.duration + description.transition.delay,
        "total duration",
    )?;
...
pub(super) fn validate(raw: &str) -> Result<(), String> {
    if raw.len() > 4096 {
        return Err("motion exceeds 4096 bytes".into());
    }
    let value = serde_json::from_str(raw).map_err(|error| format!("invalid motion: {error}"))?;
...
pub(super) fn apply(
    ...
    id: u32,
    node: &NativeNode,
    states: &mut HashMap<u32, MotionState>,
) -> Element {
    let Some(raw) = node.string(property::MOTION) else {
        states.remove(&id);
        return element;
    };
    let value: serde_json::Value = serde_json::from_str(raw).expect("validated motion JSON");
    let now = Instant::now();
    let state = states
        .entry(id)
        .or_insert_with(|| MotionState::new(&value, now).expect("validated motion"));
    state.sync(&value, now).expect("validated motion update");
    let from = state.from;
    let target = state.target;
    let transition = state.transition.clone();
    if from == target {
        return target.apply_to(element);
    }
    let total = transition.duration + transition.delay;
    let key = format!("host-motion-{id}-{}", state.generation);
    element.with_animation(
        key,
        Animation::new(Duration::from_secs_f64(total)),
        move |element, phase| {
            if phase >= 1.0 {
                return target.apply_to(element);
            }
            let elapsed = f64::from(phase) * total;
            let raw = if elapsed < transition.delay {
                0.0
            } else if transition.duration == 0.0 {
                1.0
            } else {
                ((elapsed - transition.delay) / transition.duration).clamp(0.0, 1.0)
            };
            from.interpolate(target, ease(raw, &transition.ease))
                .apply_to(element)
        },
    )
}
