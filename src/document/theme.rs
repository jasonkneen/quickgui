        ...
        let set = |slot: &mut f32, value: Option<f64>| {
            ...
            if value.is_finite() && value >= 0.0 {
                *slot = value;
                ...
                tracing::warn!("ignoring invalid theme metric {value}");
            }
        };
...
impl Theme {
    ...
    pub fn from_prop(value: Option<&serde_json::Value>) -> Self {
        ...
        match serde_json::from_value::<ThemeOverride>(value.clone()) {
            ...
            Err(e) => {
                tracing::warn!("invalid theme prop, using dark: {e}");
                Theme::dark()
            }
        }
    }
...
fn set(slot: &mut Hsla, value: &Option<String>) {
    if let Some(color) = value.as_deref().and_then(super::color::parse_color) {
        *slot = color.into();
    }
}
